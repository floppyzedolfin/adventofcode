...
func TestDec03Result_String(t *testing.T) {
	tt := map[string]struct {
		...
		"part 1": {
			result: dec03Result{treeCountPrima: common.IntPointer(4)},
			output: "The number of trees hit for Part 1 is 4.\n",
		},
		"part 2": {
			result: dec03Result{treeCountSecunda: common.IntPointer(6)},
			output: "The number of trees hit for Part 2 is 6.\n",
		},
		"parts 1 & 2": {
			result: dec03Result{treeCountPrima: common.IntPointer(4), treeCountSecunda: common.IntPointer(6)},
...
func TestDec03Result_Solve(t *testing.T) {
	tt := map[string]struct {
		...
		"nominal Prima": {
			...
			output: dec03Result{treeCountPrima: common.IntPointer(187)},
		},
		"nominal Secunda": {
			inputPath: "./input",
			parts: door.Parts{door.Secunda},
			output: dec03Result{treeCountSecunda: common.IntPointer(4723283400)},
		},
		"nominal Prima & Secunda": {
			inputPath: "./input",
			parts: door.Parts{door.Prima, door.Secunda},
			output: dec03Result{treeCountPrima: common.IntPointer(187), treeCountSecunda: common.IntPointer(4723283400)},
		},
		"no parts": {
			inputPath: "./input",
			parts: door.Parts{},
		...
		"invalid contents": {
			...
			parts: door.Parts{door.Prima},
			output: dec03Result{treeCountPrima: common.IntPointer(0)},
		},
		"no track x": {
		...
		"no track y": {
			...
			parts: door.Parts{door.Secunda},
			output: dec03Result{treeCountSecunda: common.IntPointer(0)},
		},
		"missing input": {
